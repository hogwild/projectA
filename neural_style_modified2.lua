...
-- Basic options
cmd:option('-style_image', 'examples/inputs/brad_pitt.jpg',
           'Style target image')
cmd:option('-content_image', 'examples/inputs/picasso_selfport1907.jpg',
...
local function main(params)
  ...
  local style_layers = {23}
  --local style_layer_weights = {1e0, 1e0, 1e0, 1e0, 1e0}
  local style_layer_weights = {1e0}
  ...
  for i = 1, #cnn do
    if next_content_idx <= #content_layers or next_style_idx <= #style_layers then
      ...
      if i == style_layers[next_style_idx] then
        ...
        local loss_module = nn.ContentLoss(weight,target, norm):float()
        if params.gpu >= 0 then
          loss_module:cuda()
        end
        net:add(loss_module)
