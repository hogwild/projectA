local SpatialSamplingBilinear, parent = torch.class('nn.SpatialSamplingBilinear', 'nn.Module')
...
function SpatialSamplingBilinear:__init(...)
   parent.__init(self)
   xlua.unpack_class(
      self, {...}, 'nn.SpatialSamplingBilinear', help_desc,
...
function SpatialSamplingBilinear:updateOutput(input)
   local hDim, wDim = 2, 3
   if input:dim() == 4 then
      hDim, wDim = 3, 4
   end
   ...
   input.nn.SpatialSamplingBilinear_updateOutput(self, input)
   return self.output
end

function SpatialSamplingBilinear:updateGradInput(input, gradOutput)
   input.nn.SpatialSamplingBilinear_updateGradInput(self, input, gradOutput)
